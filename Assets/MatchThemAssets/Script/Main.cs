...
public class Main : MonoBehaviour
{
    ...
    public GameObject _informationalEffect;


    [HideInInspector]
    public int _scoreTotal = 0;//The score 
    ...
    private JuicyEffect juicyEffect;
    private GameOver gameOver;
    private int _scorePerMatch = 0;

    // Stack to store transform for displaying score per match  
    ArrayList scorePoints = new ArrayList();

    void Awake()
    {
        juicyEffect = GetComponent<JuicyEffect>();
    ...
    void Start()
    {
        ...
        _stepsLeft = _steps;
        stepText.GetComponent<TextMesh>().text = _steps.ToString();
    ...
    void Update()
    {
            ...
            if (HOTween.GetTweenInfos() == null)
            {
                ...
                {  //To know if the user already selected a tile or not yet
                    ...
                    if (shouldTransit)
                    {
                        ...
                        //Update step counts
                        _stepsLeft--;
                        //Debug.Log(_stepsLeft);

                        stepText.GetComponent<TextMesh>().text = _stepsLeft.ToString();
        ...
        if (HOTween.GetTweenInfos() == null)
        {
            //Find match from current array; Stack
            var Matches = FindMatch(_arrayOfShapes);
            //If we find a matched tiles
            if (Matches.Count > 0)
            {//Update the score
                _scorePerMatch = Matches.Count * _scoreIncrement;
                _scoreTotal += _scorePerMatch;
                if (IsInformational)
                {
                    _informationalEffect.GetComponent<TextMesh>().text = _scorePerMatch.ToString();
                }

                juicyEffect.juicyScoreText(_scorePerMatch);
                ...
                foreach (GameObject go in Matches)
                {
                    ...
                    //Add informational effect


                    //Destroy the ancient matching tiles
                    Destroy(go, 0.1f);
                    //Debug.Log("Destroy matching tiles");
                }

                if (scorePoints.Count > 0 && IsInformational)
                {
                    foreach (GameObject scorePoint in scorePoints)
                    {
                        var informationalEffect = GameObject.Instantiate(_informationalEffect as GameObject, new Vector3(scorePoint.transform.position.x, scorePoint.transform.position.y + 0.5f, -2), transform.rotation) as GameObject;
                        Destroy(informationalEffect, 1f);
        ...
        if (HOTween.GetTweenInfos() == null)
        {
            gameOver.CheckSteps(_stepsLeft);
        }
        ...
        for (var x = 0; x <= cells.GetUpperBound(0); x++)
        {
            for (var y = 0; y <= cells.GetUpperBound(1); y++)
            {
                ...
                if (matchCount >= 2)
                {
                    ...
                    if (!scorePoints.Contains(cells[x, y1]))
                    {
                        scorePoints.Add(cells[x, y1]);
                    }
        ...
        for (var y = 0; y < cells.GetUpperBound(1) + 1; y++)
        {
            for (var x = 0; x < cells.GetUpperBound(0) + 1; x++)
            {
                ...
                if (matchCount >= 2)
                {
                    ...
                    if (!scorePoints.Contains(cells[x1, y]))
                    {
                        scorePoints.Add(cells[x1, y]);
                    }
